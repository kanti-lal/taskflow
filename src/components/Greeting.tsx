...
export function Greeting({ hour }: GreetingProps) {
  ...
  const getGreetingInfo = () => {
    if (hour >= 5 && hour < 12) return { text: 'Good Morning', icon: Sun, color: 'text-yellow-500' };
    if (hour >= 12 && hour < 17) return { text: 'Good Afternoon', icon: Coffee, color: 'text-orange-500' };
    if (hour >= 17 && hour < 22) return { text: 'Good Evening', icon: Sunset, color: 'text-pink-500' };
    ...
    return (
      <form onSubmit={handleSubmit} className="inline-flex items-center">
        <input
          ...
          className="px-2 py-1 text-sm border rounded dark:border-gray-700 dark:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-blue-500"
